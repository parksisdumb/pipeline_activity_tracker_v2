-- Location: supabase/migrations/20251216175100_add_master_admin_capabilities.sql
-- Schema Analysis: Existing user_role enum ('admin', 'manager', 'rep'), user_profiles table with tenant_id
-- Integration Type: Modificative - Adding super_admin role and cross-tenant access
-- Dependencies: user_profiles, tenants tables

-- Step 1: Add super_admin to existing user_role enum
...
-- Using string comparison instead of enum to avoid unsafe enum usage
...
-- Step 4: Update RLS policies on user_profiles to allow super admin access
...
-- Step 5: Add super admin policy for tenants table
...
-- Step 6: Update existing admin functions to include super admin checks
...
-- Step 7: Create master admin user
DO $$
DECLARE
...
BEGIN
    ...
    -- Create master admin profile using string casting to avoid enum issues

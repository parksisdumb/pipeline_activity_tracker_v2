-- Migration: Fix Function Return Type Conflicts and Authentication Issues
-- Date: 2025-01-14 22:00:00
-- Description: Comprehensive fix for function return type mismatches and authentication system

-- =======================================================================================
-- SECTION 1: DROP EXISTING FUNCTIONS TO AVOID TYPE CONFLICTS
-- =======================================================================================

-- Drop all existing functions to recreate with correct types
...
DROP FUNCTION IF EXISTS refresh_user_profile(TEXT);
-- =======================================================================================
-- SECTION 2: FIXED FUNCTION WITH CORRECT RETURN TYPES
-- =======================================================================================

-- CRITICAL FIX: Function to get user with complete profile using correct PostgreSQL types
CREATE OR REPLACE FUNCTION get_user_with_complete_profile(user_uuid UUID)
RETURNS TABLE (
    id UUID,
    email TEXT,  -- Changed from character varying(255) to TEXT to match PostgreSQL behavior
    role user_role,
    full_name TEXT,
    tenant_id UUID,
    is_active BOOLEAN,
...
BEGIN
    ...
    SELECT 
        up.id,
        CAST(au.email AS TEXT) as email,  -- Explicit cast to TEXT to prevent type mismatch
        up.role,
        up.full_name,
        up.tenant_id,
        ...
        CAST(t.name AS TEXT) as tenant_name,  -- Explicit cast to TEXT
        up.created_at,
        up.updated_at
...
$$;
-- =======================================================================================
-- SECTION 3: PARKS MANAGER AUTHENTICATION FIX
-- =======================================================================================
...
BEGIN
    ...
    -- Verify the setup using fixed function
...
$$;
-- =======================================================================================
-- SECTION 4: ENHANCED TENANT DATA ACCESS FUNCTIONS WITH FIXED TYPES
-- =======================================================================================
...
BEGIN
    ...
    FROM user_profiles
    WHERE id = manager_uuid AND role = 'manager'::user_role AND is_active = true;
...
END;
$$;
-- ENHANCED: Get user accessible accounts based on role (Fixed return types)
...
$$;
-- =======================================================================================
-- SECTION 5: UTILITY FUNCTIONS FOR FRONTEND
-- =======================================================================================
...
$$;
-- =======================================================================================
-- SECTION 6: EXECUTE PARKS MANAGER FIX AND TESTING
-- =======================================================================================
...
-- =======================================================================================
-- SECTION 7: UPDATE RLS POLICIES FOR COMPREHENSIVE ACCESS
-- =======================================================================================
...
-- Drop and recreate policies with corrected conditions
DROP POLICY IF EXISTS "user_profiles_comprehensive_access" ON user_profiles;
DROP POLICY IF EXISTS "accounts_comprehensive_access" ON accounts;
DROP POLICY IF EXISTS "contacts_comprehensive_access" ON contacts;
DROP POLICY IF EXISTS "properties_comprehensive_access" ON properties;
...
FOR SELECT USING (
    ...
    EXISTS (
        ...
        AND up2.role = 'manager'::user_role
...
GRANT EXECUTE ON FUNCTION sync_parks_manager_authentication() TO authenticated;
-- Final status message
SELECT json_build_object(
    'status', 'MIGRATION COMPLETED SUCCESSFULLY',
    'message', 'Function return type conflicts have been fixed and Parks manager authentication has been synchronized',

-- Location: supabase/migrations/20251226142000_fix_drop_constraint_syntax_error.sql
-- Schema Analysis: Fix syntax error in DROP CONSTRAINT statement from previous migration
-- Integration Type: Corrective - fixing SQL syntax error in authentication constraint removal
-- Dependencies: public.user_profiles, auth.users

-- CRITICAL FIX: The previous migration had incorrect DROP CONSTRAINT syntax
-- ERROR: "syntax error at or near CONSTRAINT" was caused by missing ALTER TABLE prefix

-- 1. First, correctly drop the problematic check constraint that prevents active users without tenant
ALTER TABLE public.user_profiles 
DROP CONSTRAINT IF EXISTS check_active_user_has_tenant;

...
const AddProspectModal = ({ isOpen, onClose, onProspectCreated }) => {
  ...
  const handleInputChange = (field, value) => {
    ...
    if (field === 'name' && duplicateWarning) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result?.error) {
        // Check if it's a duplicate error and show user-friendly message
        if (result?.error?.includes('already exists')) {
          setDuplicateWarning(result?.error);
          setErrors(prev => ({ ...prev, name: 'This prospect name is already in use' }));
        } else {
      ...
      {duplicateWarning && (
        <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
          <div className="flex">
            ...
            <div className="ml-3">
              <h3 className="text-sm font-medium text-yellow-800">Duplicate Prospect Name</h3>
              <p className="mt-1 text-sm text-yellow-700">{duplicateWarning}</p>
            </div>
          </div>
        </div>
      )}
